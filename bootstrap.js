...
TapTranslate = {
  ...
  cleanupUI: function(aWindow) {
    if (this._isNewContextMenu(aWindow)) {
      return delete aWindow.SelectionHandler.actions.TRANSLATE;
    } else {
  ...
  _isNewContextMenu: function(aWindow) {
    return aWindow.SelectionHandler.actions != null;
  }
};
    ...
    return Object.keys(object).forEach(function(key) {
      ...
      try {
        ...
        if (_this.isObject(value)) {
          return _this.inspect(value, "" + prefix + "{" + key + "} ");
        } else {
          return _this.log("" + prefix + key + " => (" + type + ") " + value);
        }
      } catch (_error) {
